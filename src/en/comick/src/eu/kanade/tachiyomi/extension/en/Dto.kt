...
data class DetailsPrefs(
    ...
    val matchOtherTitles: Boolean = true,
...
/**
 * Title selection for the details page. The browse/search lists use the same
 * matching + length heuristic directly, so the displayed title never changes
 * between the list and the series page.
 *
 * When "Check other titles" is enabled:
 * 1. A checked title (from the "Other titles" setting) that matches the API or
 *    an alternative title wins.
 * 2. Otherwise pick the English title closest to 12 characters (exact 12, then
 *    longer, then shorter); romanized (ja-ro/ko-ro) titles are excluded.
 */
private fun resolveTitle(
    ...
    if (prefs.matchOtherTitles) {
        return matchCheckedTitles(apiTitle, mdTitles, prefs.otherTitles)
    }
    return pickTitle(apiTitle, mdTitles, links, prefs.translatedTitle)
}
...
private fun matchCheckedTitles(
    apiTitle: String?,
    mdTitles: List<AltTitleDto>?,
    otherTitles: List<String>,
): String? {
    val romanized = mdTitles.orEmpty()
        .filter { it.lang.orEmpty().endsWith("-ro") }
        .mapNotNull { it.title?.trim()?.takeIf { title -> title.isNotBlank() } }
        .toSet()
    val candidates = buildList {
        apiTitle?.trim()?.takeIf { it.isNotBlank() && it !in romanized }?.let { add(it to "") }
        mdTitles.orEmpty().forEach { t ->
            t.title?.trim()?.takeIf { it.isNotBlank() }?.let { add(it to (t.lang ?: "")) }
        }
    }
        .filterNot { (title, lang) -> isNumericIdTitle(title) || lang.endsWith("-ro") }
        .distinctBy { it.first }
    if (candidates.isEmpty()) return apiTitle?.takeUnless { isNumericIdTitle(it) }

    for (checked in otherTitles) {
        candidates.firstOrNull { it.first.equals(checked.trim(), ignoreCase = true) }?.let { return it.first }
    }

    // Prefer English titles. When a series has no English title, fall back to
    // the API title instead of a random non-English md_title; fetching each
    // series' details page to look for one would put load on the site.
    val pool = candidates.filter { it.second.equals("en", ignoreCase = true) }
    if (pool.isEmpty()) return apiTitle?.takeUnless { isNumericIdTitle(it) }
    return pool.minWithOrNull(
        compareBy(
            { abs(it.first.length - 12) },
            { -it.first.length },
        ),
    )?.first
}
...
/**
 * Title priority when "Translated title" is on:
 * 1. Official EN link (links.engtl) slug -> human title
 * 2. English entries in md_titles (prefer shortest)
 * 3. API comic.title
 */
private fun pickTitle(
    ...
    if (!preferTranslated) return apiTitle

    val derived = linkDerivedTitles(links)
    ...
    if (enTitles.isNotEmpty()) {
        return enTitles.minByOrNull { it.length }
    }

    return apiTitle
}

/** Official-title candidates in priority order: Webtoons/Tapas (shortest first), then engtl. */
private fun linkDerivedTitles(links: JsonObject?): List<String> {
    if (links == null) return emptyList()

    fun isHttpUrl(value: String): Boolean = value.startsWith("http://", ignoreCase = true) ||
        value.startsWith("https://", ignoreCase = true)

    fun matchesPreferredHost(url: String): Boolean {
    ...
    val urls = links.mapNotNull { (_, el) ->
        (el as? JsonPrimitive)?.contentOrNull?.trim()?.takeIf { isHttpUrl(it) }
    }

    return buildList {
        val preferred = urls
            .filter { matchesPreferredHost(it) }
            .mapNotNull { titleFromOfficialUrl(it) }
        if (preferred.isNotEmpty()) {
            addAll(preferred.sortedBy { it.length })
        }
        val engtlUrl = (links["engtl"] as? JsonPrimitive)?.contentOrNull?.trim()
        titleFromOfficialUrl(engtlUrl)?.let { add(it) }
    }.distinct()
}
