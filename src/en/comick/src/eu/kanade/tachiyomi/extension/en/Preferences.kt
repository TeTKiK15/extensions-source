...
object Preferences {
    ...
    const val PREF_MATCH_OTHER_TITLES = "match_other_titles"
    const val PREF_OTHER_TITLES = "other_titles"
    ...
    fun matchOtherTitles(prefs: SharedPreferences): Boolean = prefs.getBoolean(PREF_MATCH_OTHER_TITLES, true)

    fun otherTitles(prefs: SharedPreferences): List<String> = prefs.getString(PREF_OTHER_TITLES, "")
        ?.lines()
        ?.map { it.trim() }
        ?.filter { it.isNotEmpty() }
        .orEmpty()
    ...
    fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        SwitchPreferenceCompat(screen.context).apply {
            key = PREF_MATCH_OTHER_TITLES
            title = "Check other titles"
            summary = "Use a checked title if it matches the API/alternative titles, otherwise the one closest to 12 characters"
            setDefaultValue(true)
            screen.addPreference(this)
        }

        EditTextPreference(screen.context).apply {
            key = PREF_OTHER_TITLES
            title = "Other titles"
            summary = "One title per line to check against the API and alternative titles"
            setDefaultValue("")
            dialogTitle = "Other titles"
            screen.addPreference(this)
        }
