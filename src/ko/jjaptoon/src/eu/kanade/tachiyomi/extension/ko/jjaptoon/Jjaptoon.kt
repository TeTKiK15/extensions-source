...
abstract class Jjaptoon :
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        SwitchPreferenceCompat(screen.context).apply {
            ...
            title = "English filters / 영어 필터"
            summaryOn = "Filter names are displayed in English"
            summaryOff = "필터 이름이 한국어로 표시됩니다"
        ...
        EditTextPreference(screen.context).apply {
            key = PREF_PORTAL_KEY
            title = "주소 공지 사이트"
            summary = "현재 사이트 주소를 알려주는 공지 페이지 URL"
    ...
    override suspend fun getPopularManga(page: Int): MangasPage = parseMangaList("${currentBaseUrl()}/comics?page=$page")

    // ============================== Latest ==============================

    override suspend fun getLatestUpdates(page: Int): MangasPage = parseMangaList("${currentBaseUrl()}/comics?selectedStatus=ongoing&page=$page")
        ...
        .map { element ->
            SChapter.create().apply {
                ...
                name = element.selectFirst(
                    ...
                    ?: element.absUrl("href").substringAfterLast("/")
    ...
    // Server-side pagination via ?page=N; each page holds 36 series.
    private suspend fun parseMangaList(url: String): MangasPage {
        val document = client.get(url).asJsoup()
        val currentPage = url.toHttpUrl().queryParameter("page")?.toIntOrNull() ?: 1
        ...
        val maxPage = document.select("a[href*='page=']")
            .mapNotNull { link ->
                link.absUrl("href").toHttpUrl().queryParameter("page")?.toIntOrNull()
            }
    ...
    companion object {
        ...
        private const val DEFAULT_PORTAL_URL = "https://xn--kd6b44m.net"
    }
}
